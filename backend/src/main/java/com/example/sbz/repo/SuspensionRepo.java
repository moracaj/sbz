...
public interface SuspensionRepo extends JpaRepository<Suspension, Long> {
	...
	@Query("select s from Suspension s where s.start <= :now and (s.end is null or s.end > :now)")
	List<Suspension> findActive(@Param("now") Instant now);

	@Query("select s from Suspension s where s.type = :type and s.start <= :now and (s.end is null or s.end > :now)")
	List<Suspension> findActiveByType(@Param("type") SuspensionType type, @Param("now") Instant now);
...
public interface SuspensionRepo extends JpaRepository<Suspension, Long> {
  ...
  // Aktivan = (startAt je null ili <= now) I (endAt je null ili > now)
  @Query("""
         select s
         from Suspension s
         where (s.startAt is null or s.startAt <= :now)
